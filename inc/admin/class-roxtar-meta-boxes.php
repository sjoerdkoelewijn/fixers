 ...
 * Meta Boxes setup
 */
if ( ! class_exists( 'Roxtar_Meta_Boxes' ) ) {

	/**
	 * Meta Boxes setup
	 */
	class Roxtar_Meta_Boxes {
		...
		public function __construct() {
			add_action( 'load-post.php', array( $this, 'roxtar_init_metabox' ) );
			add_action( 'load-post-new.php', array( $this, 'roxtar_init_metabox' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'roxtar_metabox_assets' ) );
		}

		/**
		 *  Init Metabox
		 */
		public function roxtar_init_metabox() {

			add_action( 'add_meta_boxes', array( $this, 'roxtar_setup_meta_box' ) );
			add_action( 'save_post', array( $this, 'roxtar_save_meta_box' ) );
		 ...
		 */
		public function roxtar_metabox_assets() {
			wp_enqueue_style(
				'roxtar-metabox-setting',
				ROXTAR_THEME_URI . 'assets/css/admin/metabox.css',
				array(),
				roxtar_version()
			);
		}
		 ...
		 *  Setup Metabox
		 */
		public function roxtar_setup_meta_box() {

			// Get all public posts.
			$post_types = apply_filters(
				'roxtar_metabox_post_types',
				array( 'post', 'page' )
			);

			// Enable for all posts.
			foreach ( $post_types as $type ) {
				$metabox_name = ucwords( $type ) . __( ' Settings', 'roxtar' );

				add_meta_box(
					'roxtar_metabox_settings_general',           // Id.
					$metabox_name,                          // Title.
					array( $this, 'roxtar_markup_meta_box' ),      // Callback.
					$type,                                  // Post_type.
					'side'                             // Context.
				);
			}
		 ...
		 * Get metabox options
		 */
		public static function roxtar_get_meta_option() {
		 ...
		 * @return void
		 */
		public function roxtar_markup_meta_box( $post ) {

			wp_nonce_field( basename( __FILE__ ), 'roxtar_metabox_settings_general' );
			...
			// Get defaults.
			$meta = self::roxtar_get_meta_option();
			...
			<div class="roxtar-metabox-setting">
				...
				<div class="roxtar-metabox-option">
					<div class="roxtar-metabox-option-title">
						<span><?php esc_html_e( 'Container', 'roxtar' ); ?>:</span>
					</div>
				...
				<div class="roxtar-metabox-option">
					<div class="roxtar-metabox-option-title">
						<span><?php esc_html_e( 'Sidebar', 'roxtar' ); ?>:</span>
					</div>
					...
					<div class="roxtar-metabox-option-content">
						<select name="site-sidebar" id="site-sidebar">
							...
							<option value="left" <?php selected( $site_sidebar, 'left' ); ?> >
								<?php esc_html_e( 'Left Sidebar', 'roxtar' ); ?>
							</option>

							<option value="right" <?php selected( $site_sidebar, 'right' ); ?> >
								<?php esc_html_e( 'Right Sidebar', 'roxtar' ); ?>
							</option>

							<option value="full" <?php selected( $site_sidebar, 'full' ); ?> >
								<?php esc_html_e( 'No Sidebar', 'roxtar' ); ?>
							</option>
						</select>
					</div>
				</div>
				...
				<div class="roxtar-metabox-option">
					<div class="roxtar-metabox-option-title">
						<span><?php esc_html_e( 'Transparent Header', 'roxtar' ); ?>:</span>
					</div>
				...
				<div class="roxtar-metabox-option">
					<div class="roxtar-metabox-option-title">
						<span><?php esc_html_e( 'Page Header', 'roxtar' ); ?>:</span>
					</div>
				...
				<div class="roxtar-metabox-option">
					<div class="roxtar-metabox-option-title">
						<span><?php esc_html_e( 'Disable Sections', 'roxtar' ); ?>:</span>
					</div>
						...
						<div class="disable-section-meta">
							<div class="site-topbar-option-wrap">
								<label for="site-topbar">
									...
									<?php esc_html_e( 'Disable Topbar', 'roxtar' ); ?>
								</label>
							</div>
							...
							<div class="site_header-option-wrap">
								<label for="site_header">
									...
									<?php esc_html_e( 'Disable Header', 'roxtar' ); ?>
								</label>
							</div>
							...
							<div class="site-footer-option-wrap">
								<label for="site-footer">
									...
									<?php esc_html_e( 'Disable Footer', 'roxtar' ); ?>
								</label>
							</div>
						</div>
					</div>
		 ...
		 * @return void
		 */
		public function roxtar_save_meta_box( $post_id ) {

			// Checks save status.
			$is_autosave    = wp_is_post_autosave( $post_id );
			$is_revision    = wp_is_post_revision( $post_id );
			$is_valid_nonce = ( isset( $_POST['roxtar_metabox_settings_general'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['roxtar_metabox_settings_general'] ) ), basename( __FILE__ ) ) ) ? true : false;
			 ...
			 * Get meta options
			 */
			$post_meta = self::roxtar_get_meta_option();
	 ...
	 */
	Roxtar_Meta_Boxes::get_instance();
}
