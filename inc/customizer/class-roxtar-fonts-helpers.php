...
if ( ! class_exists( 'Roxtar_Fonts_Helpers' ) ) :

	/**
	 * The Roxtar Customizer class
	 */
	class Roxtar_Fonts_Helpers {
		...
		public function __construct() {
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'roxtar_do_control_inline_scripts' ), 100 );
			add_action( 'wp_enqueue_scripts', array( $this, 'roxtar_enqueue_google_fonts' ), 0 );
			add_action( 'enqueue_block_editor_assets', array( $this, 'roxtar_enqueue_google_fonts' ), 0 );
			add_action( 'wp_ajax_roxtar_get_all_google_fonts_ajax', array( $this, 'roxtar_get_all_google_fonts_ajax' ) );
			add_filter( 'roxtar_typography_customize_list', array( $this, 'roxtar_add_to_font_customizer_list' ) );

			// Add custom font css.
			add_action( 'wp_head', array( $this, 'roxtar_add_custom_font_style' ) );
			if ( is_admin() ) {
				add_action( 'enqueue_block_assets', array( $this, 'roxtar_add_custom_font_style' ) );
			}
		}
		 ...
		 * Add custom font css
		 */
		public function roxtar_add_custom_font_style() {
		 ...
		 */
		public function roxtar_do_control_inline_scripts() {
			wp_localize_script(
				'roxtar-typography-customizer',
				'roxtar_customize',
				array(
					'nonce' => wp_create_nonce( 'roxtar_customize_nonce' ),
				)
			);
			...
			wp_localize_script(
				'roxtar-typography-customizer',
				'typography_defaults',
				$this->roxtar_typography_default_fonts()
			);
		}
		 ...
		 */
		public static function roxtar_typography_default_fonts() {
			$fonts = array(
				'inherit',
				'System Stack',
				'Arial, sans-serif',
			...
			return apply_filters( 'roxtar_typography_default_fonts', $fonts );
		}
		 ...
		 */
		public static function roxtar_get_default_fonts( $filter = true ) {
			$roxtar_font_defaults = array(
				// Body font.
			...
			if ( $filter ) {
				return apply_filters( 'roxtar_font_option_defaults', $roxtar_font_defaults );
			}

			return $roxtar_font_defaults;
		}

		/**
		 * Add Google Fonts to wp_head if needed.
		 */
		public function roxtar_enqueue_google_fonts() {
			// Grab our options.
			$roxtar_settings = wp_parse_args(
				get_option( 'roxtar_setting', array() ),
				self::roxtar_get_default_fonts()
			);

			// List our non-Google fonts.
			$not_google = str_replace( ' ', '+', self::roxtar_typography_default_fonts() );
			...
			if ( ! empty( $font_settings ) ) {
				...
				foreach ( $font_settings as $key ) {
					...
					if ( ! isset( $roxtar_settings[ $key ] ) ) {
						continue;
					}

					// If our value is still using the old format, fix it.
					if ( strpos( $roxtar_settings[ $key ], ':' ) !== false ) {
						$roxtar_settings[ $key ] = current( explode( ':', $roxtar_settings[ $key ] ) );
					}

					// Replace the spaces in the names with a plus.
					$value = str_replace( ' ', '+', $roxtar_settings[ $key ] );

					// Grab the variants using the plain name.
					$variants = $this->roxtar_get_google_font_variants( $roxtar_settings[ $key ], $key );
			...
			$google_fonts = apply_filters( 'roxtar_typography_google_fonts', $google_fonts );

			// Get the subset.
			$subset = apply_filters( 'roxtar_fonts_subset', '' );
		 ...
		 */
		public static function roxtar_get_all_google_fonts( $amount = 'all' ) {
			...
			if ( apply_filters( 'roxtar_alphabetize_google_fonts', true ) ) {
				asort( $fonts );
			}

			// Filter to allow us to modify the fonts array.
			return apply_filters( 'roxtar_google_fonts_array', $fonts );
		}

		/**
		 * Return an array of all of our Google Fonts.
		 */
		public function roxtar_get_all_google_fonts_ajax() {
			/*Do another nonce check*/
			check_ajax_referer( 'roxtar_customize_nonce', 'roxtar_customize_nonce' );

			/*Bail if the nonce doesn't check out*/
			if (
				! isset( $_POST['roxtar_customize_nonce'] ) ||
				! wp_verify_nonce( sanitize_key( $_POST['roxtar_customize_nonce'] ), 'roxtar_customize_nonce' ) ||
			...
			$fonts = $this->roxtar_get_all_google_fonts();
		 ...
		 * Example: roxtar_get_google_font_category( 'Open Sans' )
		 *
		 * @param string $font The name of our font.
		 * @param string $key The ID of the font setting.
		 * @return string The category of our font.
		 */
		public static function roxtar_get_google_font_category( $font, $key = '' ) {
			// Don't need a category if we're using a system font.
			if ( in_array( $font, self::roxtar_typography_default_fonts(), true ) ) {
				return;
			}
			...
			// Get our defaults.
			$defaults = self::roxtar_get_default_fonts();
			...
			$fonts = self::roxtar_get_all_google_fonts();
		 ...
		 * Example: roxtar_get_google_font_variation( 'Open Sans' )
		 *
		 * @param      string $font   The font.
		 * @param      string $key    The key.
		 */
		public function roxtar_get_google_font_variants( $font, $key = '' ) {
			...
			if ( $variants ) {
				...
				return implode( ',', apply_filters( 'roxtar_typography_variants', $output ) );
			}
		}
		 ...
		 */
		public function roxtar_add_to_font_customizer_list( $fonts ) {
			// Bail if we don't have our defaults.
			$roxtar_settings = wp_parse_args(
				get_option( 'roxtar_settings', array() ),
				self::roxtar_get_default_fonts()
			);
			...
			$all_fonts    = $this->roxtar_get_all_google_fonts();
			$select_fonts = $this->roxtar_get_all_google_fonts( apply_filters( 'roxtar_number_of_fonts', 200 ) );

			foreach ( $font_settings as $setting ) {
				// If we don't have a setting, keep going.
				if ( ! isset( $roxtar_settings[ $setting ] ) ) {
					continue;
				}

				$id = strtolower( str_replace( ' ', '_', $roxtar_settings[ $setting ] ) );

				if ( array_key_exists( $id, $select_fonts ) || in_array( $roxtar_settings[ $setting ], self::roxtar_typography_default_fonts(), true ) ) {
					continue;
				}

				$fonts[ strtolower( str_replace( ' ', '_', $roxtar_settings[ $setting ] ) ) ] = array(
					'name'     => $roxtar_settings[ $setting ],
		 ...
		 */
		public static function roxtar_get_font_family_css( $font, $settings, $default ) {
			$roxtar_settings = wp_parse_args(
			...
			$no_quotes = array(
				...
				'Trebuchet MS, Helvetica, sans-serif',
				'Verdana, Geneva, sans-serif',
				apply_filters( 'roxtar_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' ),
			);

			/*Get our font*/
			$font_family = $roxtar_settings[ $font ];

			if ( 'System Stack' === $font_family ) {
				$font_family = apply_filters( 'roxtar_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
			}
			...
			if ( in_array( $font_family, $no_quotes, true ) ) {
				...
				$wrapper_start = '"';
				$wrapper_end   = '"' . self::roxtar_get_google_font_category( $font_family, $font );
			}
...
Roxtar_Fonts_Helpers::get_instance();
