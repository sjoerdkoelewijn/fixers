...
if ( ! class_exists( 'SKDD_Fonts_Helpers' ) ) :

	/**
	 * The SKDD Customizer class
	 */
	class SKDD_Fonts_Helpers {
		...
		public function __construct() {
			add_action( 'customize_controls_enqueue_scripts', array( $this, 'SKDD_do_control_inline_scripts' ), 100 );
			add_action( 'wp_enqueue_scripts', array( $this, 'SKDD_enqueue_google_fonts' ), 0 );
			add_action( 'enqueue_block_editor_assets', array( $this, 'SKDD_enqueue_google_fonts' ), 0 );
			add_action( 'wp_ajax_SKDD_get_all_google_fonts_ajax', array( $this, 'SKDD_get_all_google_fonts_ajax' ) );
			add_filter( 'SKDD_typography_customize_list', array( $this, 'SKDD_add_to_font_customizer_list' ) );

			// Add custom font css.
			add_action( 'wp_head', array( $this, 'SKDD_add_custom_font_style' ) );
			if ( is_admin() ) {
				add_action( 'enqueue_block_assets', array( $this, 'SKDD_add_custom_font_style' ) );
			}
		}
		 ...
		 * Add custom font css
		 */
		public function SKDD_add_custom_font_style() {
		 ...
		 */
		public static function SKDD_typography_default_fonts() {
			$fonts = array(
				'inherit',
				'System Stack',
				'Arial, sans-serif',
			...
			return apply_filters( 'SKDD_typography_default_fonts', $fonts );
		}
		 ...
		 */
		public static function SKDD_get_default_fonts( $filter = true ) {
			$SKDD_font_defaults = array(
				// Body font.
				'body_font_family'             => 'Poppins',
				'body_font_family_variants'    => '',
				'body_font_size'               => 20,
				'body_font_size_tablet'        => 18,
				'body_font_size_mobile'        => 16,
				'body_font_weight'             => 400,
				'body_line_height'             => 35,
				'body_line_height_tablet'      => 32,
				'body_line_height_mobile'      => 28,
				'body_font_category'           => '',
				...
				'parent_menu_line_height'        => 60,
				'parent_menu_line_height_tablet' => 50,
				'parent_menu_line_height_mobile' => 40,
				'sub_menu_font_size'             => 18,
				'sub_menu_font_size_tablet'      => 17,
				'sub_menu_font_size_mobile'      => 16,
				'sub_menu_line_height'           => 30,
				'sub_menu_line_height_tablet'    => 25,
				'sub_menu_line_height_mobile'    => 22,
				'menu_font_category'             => '',
			...
			if ( $filter ) {
				return apply_filters( 'SKDD_font_option_defaults', $SKDD_font_defaults );
			}

			return $SKDD_font_defaults;
		}

		/**
		 * Add Google Fonts to wp_head if needed.
		 */
		public function SKDD_enqueue_google_fonts() {
			// Grab our options.
			$SKDD_settings = wp_parse_args(
				get_option( 'SKDD_setting', array() ),
				self::SKDD_get_default_fonts()
			);

			// List our non-Google fonts.
			$not_google = str_replace( ' ', '+', self::SKDD_typography_default_fonts() );
			...
			if ( ! empty( $font_settings ) ) {
				...
				foreach ( $font_settings as $key ) {
					...
					if ( ! isset( $SKDD_settings[ $key ] ) ) {
						continue;
					}

					// If our value is still using the old format, fix it.
					if ( strpos( $SKDD_settings[ $key ], ':' ) !== false ) {
						$SKDD_settings[ $key ] = current( explode( ':', $SKDD_settings[ $key ] ) );
					}

					// Replace the spaces in the names with a plus.
					$value = str_replace( ' ', '+', $SKDD_settings[ $key ] );

					// Grab the variants using the plain name.
					$variants = $this->SKDD_get_google_font_variants( $SKDD_settings[ $key ], $key );
			...
			$google_fonts = apply_filters( 'SKDD_typography_google_fonts', $google_fonts );

			// Get the subset.
			$subset = apply_filters( 'SKDD_fonts_subset', '' );
		...
		public static function SKDD_get_all_google_fonts( $amount = 'all' ) {
			...
			if ( apply_filters( 'SKDD_alphabetize_google_fonts', true ) ) {
				asort( $fonts );
			}

			// Filter to allow us to modify the fonts array.
			return apply_filters( 'SKDD_google_fonts_array', $fonts );
		}

		/**
		 * Return an array of all of our Google Fonts.
		 */
		public function SKDD_get_all_google_fonts_ajax() {
			/*Do another nonce check*/
			check_ajax_referer( 'SKDD_customize_nonce', 'SKDD_customize_nonce' );

			/*Bail if the nonce doesn't check out*/
			if (
				! isset( $_POST['SKDD_customize_nonce'] ) ||
				! wp_verify_nonce( sanitize_key( $_POST['SKDD_customize_nonce'] ), 'SKDD_customize_nonce' ) ||
			...
			$fonts = $this->SKDD_get_all_google_fonts();
		...
		public static function SKDD_get_google_font_category( $font, $key = '' ) {
			...
			// Get our defaults.
			$defaults = self::SKDD_get_default_fonts();
			...
			$fonts = self::SKDD_get_all_google_fonts();
		 ...
		 * Example: SKDD_get_google_font_variation( 'Open Sans' )
		 *
		 * @param      string $font   The font.
		 * @param      string $key    The key.
		 */
		public function SKDD_get_google_font_variants( $font, $key = '' ) {
			...
			if ( $variants ) {
				...
				return implode( ',', apply_filters( 'SKDD_typography_variants', $output ) );
			}
		}
		 ...
		 */
		public function SKDD_add_to_font_customizer_list( $fonts ) {
			// Bail if we don't have our defaults.
			$SKDD_settings = wp_parse_args(
				get_option( 'SKDD_settings', array() ),
				self::SKDD_get_default_fonts()
			);
			...
			$all_fonts    = $this->SKDD_get_all_google_fonts();
			$select_fonts = $this->SKDD_get_all_google_fonts( apply_filters( 'SKDD_number_of_fonts', 1000 ) );

			foreach ( $font_settings as $setting ) {
				// If we don't have a setting, keep going.
				if ( ! isset( $SKDD_settings[ $setting ] ) ) {
					continue;
				}

				$id = strtolower( str_replace( ' ', '_', $SKDD_settings[ $setting ] ) );

				if ( array_key_exists( $id, $select_fonts ) || in_array( $SKDD_settings[ $setting ], self::SKDD_typography_default_fonts(), true ) ) {
					continue;
				}

				$fonts[ strtolower( str_replace( ' ', '_', $SKDD_settings[ $setting ] ) ) ] = array(
					'name'     => $SKDD_settings[ $setting ],
		 ...
		 */
		public static function SKDD_get_font_family_css( $font, $settings, $default ) {
			$SKDD_settings = wp_parse_args(
			...
			$no_quotes = array(
				...
				'Trebuchet MS, Helvetica, sans-serif',
				'Verdana, Geneva, sans-serif',
				apply_filters( 'SKDD_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' ),
			);

			/*Get our font*/
			$font_family = $SKDD_settings[ $font ];

			if ( 'System Stack' === $font_family ) {
				$font_family = apply_filters( 'SKDD_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
			}
			...
			if ( in_array( $font_family, $no_quotes, true ) ) {
				...
				$wrapper_start = '"';
				$wrapper_end   = '"' . self::SKDD_get_google_font_category( $font_family, $font );
			}
...
SKDD_Fonts_Helpers::get_instance();
